...
Blockchain.prototype.hashBlock = function(previousBlockHash, currentBlockData, nonce){
    //pascalCase = barryCumbie lower first word, upper each word, no spaces
    //CamelCase = BarryCumbie
    //jebob-case = barry-cumbie
    //snake_case = barri_andrew_cumbie
    
    //smoosh all of our 3 parameters into one long string
    const dataAsString = previousBlockHash + nonce.toString() + currentBlockData + JSON.stringify(currentBlockData);
    
    //Pass all of our data as a string into our new, cool js-sha256 library
    ...
    //What does this method do? Take de blockdata ==> some hash result
...
Blockchain.prototype.proofOfWork = function(previousBlockHash, currentBlockData){
    //hashing = previus block + curernt block + onnce
    //set our difficulty level: hash has to start with 0 or 00 or 000 or 0000
    //brute force, increment nonce & return the hash until we are happy (happy = hash satisfies the business rule/difficulty lvl)

    let nonce = 0; //start are zero, use the let keywork 'cuz this is gonna change
    ...
    // = mean to assign a value. sha256 = require('js-sha256');
    // == comparison. does x == y, are they the same, result: true or false
